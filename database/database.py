...
class databaseCon:
  ...
  def getAlbumDB(self, album, ret=False, db_artistid=None):
    return self.selectUpdateInsert(
      ...
      update_stm_str = ("UPDATE albums SET spotify_popularity = $2,lastfm_listeners = $3,lastfm_playcount = $4,whatcd_seeders = $5,whatcd_snatches = $6,pitchfork_rating = $7, kups_playcount = $8, popularity=$9"
        +(", folder_path = $11" if len(album.filepath)>0 else "")
        +" FROM artists_albums WHERE artists_albums.album_id = albums.album_id AND album = $1 and artists_albums.artist_id = $10"),
      select_args = ['name'],
      ...
      uargs = ([self.db_res['artist'][0]['select'][0] if db_artistid is None else db_artistid])+([album.filepath] if len(album.filepath)>0 else []),
      update_args = ['name','spotify_popularity','lastfm_listeners','lastfm_playcount','whatcd_seeders','whatcd_snatches','pitchfork_rating','kups_playcount','popularity']
      )
  ...
  def getSongsDB(self, songs, ret=False, db_albumid=None):
    return self.selectUpdateInsert(
      ...
      update_stm_str = ("UPDATE songs SET explicit=$2, spotify_popularity =$3,lastfm_listeners = $4,lastfm_playcount = $5,kups_playcount = $6,popularity = $7"
      +(", filename = $8" if len(songs)>0 and max([len(song.filename) for song in songs])>0 else "")
        +" WHERE song = $1"),
      select_args = ['name'],
      ...
      + (['filename'] if len(songs)>0 and max([len(song.filename) for song in songs])>0 else []))
    )
