...
struct id3_2_3_file {
  TFILE *iofile;
  enum id3_2_3_file_mode mode;
  char encoding[64];
  char *path;

  int flags;

  struct id3_2_3_tag *primary;
...
enum {
  ID3_2_3_FILE_FLAG_ID3V1 = 0x0001
};
...
{
  long save_position;
  id3_2_3_byte_t query[ID3_2_3_TAG_QUERYSIZE];
  signed long size;

  save_position = ttell(iofile);
  size = id3_2_3_tag_query(query, tread(query, 1, sizeof(query), iofile));
 ...
 */
static
struct id3_2_3_tag *read_tag(TFILE *iofile, id3_2_3_length_t size)
{
  id3_2_3_byte_t *data;
  struct id3_2_3_tag *tag = 0;

  data = malloc(size);
  if (data) {
    if (tread(data, size, 1, iofile) == 1)
      tag = id3_2_3_tag_parse(data, size);
 ...
 */
static
int update_primary(struct id3_2_3_tag *tag, struct id3_2_3_tag const *new)
{
  unsigned int i;
  struct id3_2_3_frame *frame, *genreframe = NULL;
  int hasGenre = 0;

  if (new) {
      if (!(new->extendedflags & ID3_2_3_TAG_EXTENDEDFLAG_TAGISANUPDATE))
      {
      ...
      i = 0;
      while ((frame = id3_2_3_tag_findframe(new, 0, i++))) 
      {
          if (strcmp(frame->id, "TCON") == 0)
              hasGenre = 1;
 ...
 */
static
int add_filetag(struct id3_2_3_file *file, struct filetag const *filetag)
{
  struct filetag *tags;
 ...
 */
static
void del_filetag(struct id3_2_3_file *file, unsigned int index)
{
 ...
 */
static
struct id3_2_3_tag *add_tag(struct id3_2_3_file *file, id3_2_3_length_t length)
{
  long location;
  unsigned int i;
 ...
 */
static
int search_tags(struct id3_2_3_file *file)
{
  long save_position;
  signed long size;
  ...
  if (tseek(file->iofile, -128, SEEK_END) == 0) {
    ...
    if (size > 0) {
      struct id3_2_3_tag const *tag;

      tag = add_tag(file, size);

      /* if this is indeed an ID3v1 tag, mark the file so */

      if (tag && (ID3_2_3_TAG_VERSION_MAJOR(id3_2_3_tag_version(tag)) == 1))
	file->flags |= ID3_2_3_FILE_FLAG_ID3V1;
    }
  }
  ...
  if (size > 0) {
    struct id3_2_3_tag const *tag;
    struct id3_2_3_frame const *frame;

    tag = add_tag(file, size);

    /* locate tags indicated by SEEK frames */

    while (tag && (frame = id3_2_3_tag_findframe(tag, "SEEK", 0))) {
      long seek;

      seek = id3_2_3_field_getint(id3_2_3_frame_field(frame, 0));
  ...
  if (tseek(file->iofile, ((file->flags & ID3_2_3_FILE_FLAG_ID3V1) ? -128 : 0) +
	    -10, SEEK_END) == 0) {
    size = query_tag(file->iofile);
  ...
  if ((file->ntags > 0 && !(file->flags & ID3_2_3_FILE_FLAG_ID3V1)) ||
      (file->ntags > 1 &&  (file->flags & ID3_2_3_FILE_FLAG_ID3V1))) {
    if (file->tags[0].location == 0)
      id3_2_3_tag_setlength(file->primary, file->tags[0].length);
    else
      id3_2_3_tag_options(file->primary, ID3_2_3_TAG_OPTION_APPENDEDTAG, ~0);
  }
 ...
 */
static
void finish_file(struct id3_2_3_file *file)
{
  unsigned int i;
  ...
  if (file->primary) {
    id3_2_3_tag_delref(file->primary);
    id3_2_3_tag_delete(file->primary);
  }
 ...
 */
static
struct id3_2_3_file *new_file(TFILE *iofile, enum id3_2_3_file_mode mode,
			  char const *path, const char *encoding)
{
  ...
  file->primary = id3_2_3_tag_new();
  if (file->primary == 0)
    goto fail;

  id3_2_3_tag_addref(file->primary);
  ...
  id3_2_3_tag_options(file->primary, ID3_2_3_TAG_OPTION_ID3V1,
		  (file->flags & ID3_2_3_FILE_FLAG_ID3V1) ? ~0 : 0);
 ...
 */
struct id3_2_3_file *id3_2_3_file_open(char const *path, enum id3_2_3_file_mode mode, const char *encoding)
{
  TFILE *iofile;
  struct id3_2_3_file *file;
  ...
  iofile = topen(path, (mode == ID3_2_3_FILE_MODE_READWRITE) ? "r+b" : "rb", encoding);
  if (iofile == 0)
    return 0;
 ...
 */
int id3_2_3_file_close(struct id3_2_3_file *file)
{
  int result = 0;
 ...
 */
struct id3_2_3_tag *id3_2_3_file_tag(struct id3_2_3_file const *file)
{
  assert(file);
 ...
 */
static
int v1_write(struct id3_2_3_file *file,
	     id3_2_3_byte_t const *data, id3_2_3_length_t length)
{
  ...
  if (data) {
    ...
    if (!(file->flags & ID3_2_3_FILE_FLAG_ID3V1)) {
      struct filetag filetag;
 ...
 */
static
int v2_write(struct id3_2_3_file *file,
	     id3_2_3_byte_t const *data, id3_2_3_length_t length)
{
  TFILE  *out;
  char  *newpath = NULL, *buffer = NULL, *oldpath = NULL;
  int    numread = 0, numwritten = 0;
  struct id3_2_3_file *newfile = NULL;
  long offset = 0;

  assert(!data || length > 0);

  if (((file->ntags == 1 && !(file->flags & ID3_2_3_FILE_FLAG_ID3V1)) ||
       (file->ntags == 2 &&  (file->flags & ID3_2_3_FILE_FLAG_ID3V1))) &&
  ...
  if (file->tags && ID3_2_3_TAG_VERSION_MAJOR(file->tags[0].tag->version) > 1)
  ...
  newfile = id3_2_3_file_open(file->path, file->mode, file->encoding);
  if (newfile)
  {
      int i;
      ...
      memcpy(file, newfile, sizeof(struct id3_2_3_file));
  }
  else
  {
      /* Hmmm. Something is wrong. Let's zero out the current info */
      memset(file, 0, sizeof(struct id3_2_3_file));
  }
 ...
 */
int id3_2_3_file_update(struct id3_2_3_file *file)
{
  int options, result = 0;
  id3_2_3_length_t v1size = 0, v2size = 0;
  id3_2_3_byte_t id3v1_data[128], *id3v1 = 0, *id3v2 = 0;

  assert(file);

  if (file->mode != ID3_2_3_FILE_MODE_READWRITE)
    return -1;

  options = id3_2_3_tag_options(file->primary, 0, 0);

  /* render ID3v1 */

  if (options & ID3_2_3_TAG_OPTION_ID3V1) {
    v1size = id3_2_3_tag_render(file->primary, 0);
    if (v1size) {
      assert(v1size == sizeof(id3v1_data));

      v1size = id3_2_3_tag_render(file->primary, id3v1_data);
      if (v1size) {
	assert(v1size == sizeof(id3v1_data));
	id3v1 = id3v1_data;
      }
  ...
  id3_2_3_tag_options(file->primary, ID3_2_3_TAG_OPTION_ID3V1, 0);

  v2size = id3_2_3_tag_render(file->primary, 0);
  if (v2size) {
    id3v2 = malloc(v2size);
    if (id3v2 == 0)
      goto fail;

    v2size = id3_2_3_tag_render(file->primary, id3v2);
    if (v2size == 0) {
      free(id3v2);
      id3v2 = 0;
    }
  ...
  id3_2_3_tag_options(file->primary, ~0, options);
