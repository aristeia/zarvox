...
def analyzeFolder(path_to_album, extension):
  downloads = []
...
def checkoutFolder(path_to_album):
  ...
  if os.path.isfile(path_to_album+".metadata.json"):
    ...
    if len(extensions)>0:
      ...
      analyzeFolder(path_to_album, extension)
    else:
      raise RuntimeError("Error: cannot get extension")

def main():
  checkoutFolder(getConfig()['albums_folder']+'/'+sys.argv[1].strip('/') + '/')
